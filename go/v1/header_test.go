...
func TestHeader_Pack(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			bytes: []byte{0b00000001, 0b00000001, 0, 0, 0, 1, 0, 0xff, 0, 0, 7},
		},
		{
			label: "pack response should ok",
			header: Header{
				Type:       uint8(ResponsePacket),
				Verify:     1,
				Gzip:       1,
				Reserve:    0,
				CmdCode:    1,
			...
			bytes: []byte{0b00110010, 1, 0, 0, 0, 1, 8, 0, 0, 7},
		},
		{
			label: "pack push should ok",
			header: Header{
				Type:       uint8(PushPacket),
				Verify:     0,
				Gzip:       1,
				Reserve:    3,
				CmdCode:    1,
	...
	for _, c := range cases {
		...
		t.Run(c.label, func(t *testing.T) {
			...
			switch PacketType(c.header.Type) {
			case RequestPacket:
...
func TestHeader_UnpackBytes(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			bytes: []byte{0b00000001, 0b00000001, 0, 0, 0, 1, 0, 0xff, 0, 0, 7, 1, 2, 3},
			body:  []byte{1, 2, 3},
			ctx: &protocol.Context{
				Context: context.Background(),
			},
		},
		{
			...
			bytes: []byte{0b00110010, 1, 0, 0, 0, 1, 8, 0, 0, 7, 5, 6, 7},
			body:  []byte{5, 6, 7},
			ctx: &protocol.Context{
				Context: context.Background(),
			},
		},
		{
			...
			bytes: []byte{0b11100011, 1, 0, 0, 7, 1, 1, 1},
			body:  []byte{1, 1, 1},
		},
		{
			label: "invalid packet type",
